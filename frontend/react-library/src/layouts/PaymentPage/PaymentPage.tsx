...
export default function PaymentPage() {
    ...
    useEffect(() => {
        const fetchFees = async () => {
            if (authState && authState.isAuthenticated) {
                ...
                const paymentResponse = await fetch(url, requestionOptions);
    ...
    async function checkout() {
        ...
        const url = `https://localhost:8000/api/payment/secure/payment-intent`;
        const requestionOptions = {
            method: 'POST',
            headers: {
        ...
        const stripeResponse = await fetch(url, requestionOptions);
        ...
        stripe.confirmCardPayment(
            ...
            if (result.error) {
                ...
                const url = `https://localhost:8000/api/payment/secure/payment-complete`;
                const requestionOptions = {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
